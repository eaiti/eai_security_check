...
export class DaemonOperations {
   ...
   */
  static async setupDaemonAutomation(options: DaemonSetupOptions = {}): Promise<void> {
    ...
    const setupService = options.autoSetup ?? (await ConfigManager.promptForDaemonSetup());
    if (setupService) {
...
<dict>
   ...
   */
  static async manageDaemonService(options: DaemonManagementOptions): Promise<void> {
