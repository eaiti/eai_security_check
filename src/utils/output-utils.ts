...
export class OutputUtils {
    ...
    try {
      ...
      if (platform === "darwin") {
        ...
        try {
          ...
          try {
            await execAsync(
              `echo "${content.replace(/"/g, '\\"')}" | xsel --clipboard --input`,
            );
          } catch {
  ...
  private static convertToMarkdown(report: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Convert headers
      if (
        line.includes("Security Audit Report") ||
        line.includes("EAI Security Check")
      ) {
        continue; // Skip, we already have a header
      } else if (line.includes("==") || line.includes("--")) {
        continue; // Skip separator lines
      } else if (
        line.includes("SECURITY CHECKS:") ||
  ...
  private static formatForEmail(report: string): string {
    ...
    emailBody += "\n" + "=".repeat(60) + "\n\n";
    emailBody +=
      "This report was generated automatically by the EAI Security Check tool.\n";
