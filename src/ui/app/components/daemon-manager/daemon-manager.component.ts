  ...
  template: `
    <div class="daemon-manager-container">
      ...
      <div class="controls">
        ...
        <div class="system-service-controls">
          <h3>System Service Integration</h3>
          <p>Setup the daemon as a system service for automatic startup</p>

          @if (systemServiceStatus()) {
            <div class="service-status">
              <div class="status-item">
                <span class="label">System Service:</span>
                <span
                  class="value"
                  [class]="
                    'status-' +
                    (systemServiceStatus()!.installed
                      ? 'installed'
                      : 'not-installed')
                  "
                >
              ...
              @if (
                systemServiceStatus()!.installed &&
                systemServiceStatus()!.running !== undefined
              ) {
                <div class="status-item">
                  ...
                  <span
                    ...
                    [class]="
                      'status-' +
                      (systemServiceStatus()!.running ? 'running' : 'stopped')
                    "
                  >
                    {{
                      systemServiceStatus()!.running
                        ? "🟢 Running"
                        : "🔴 Stopped"
                    }}
                  </span>
          ...
          <div class="service-buttons">
            <button
              ...
              [disabled]="isOperating() || systemServiceStatus()?.installed"
              (click)="setupSystemService()"
            >
            ...
            <button
              ...
              [disabled]="isOperating() || !systemServiceStatus()?.installed"
              (click)="removeSystemService()"
            >
              🗑️ Remove System Service
            </button>
          </div>
        </div>
      </div>
                ...
                <div class="collapsible-content">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" [(ngModel)]="smtpSecureInput" />
...
export class DaemonManagerComponent implements OnInit {
  ...
  private readonly _systemServiceStatus = signal<SystemServiceStatus | null>(
    null,
  );
  private readonly _daemonLogs = signal<string[]>([]);
  private readonly _isOperating = signal(false);
  private readonly _message = signal<string>("");
  private readonly _messageType = signal<"success" | "error" | "info">("info");

  readonly status = this._status.asReadonly();
  readonly systemServiceStatus = this._systemServiceStatus.asReadonly();
  readonly daemonLogs = this._daemonLogs.asReadonly();
  ...
  async restartDaemon(): Promise<void> {
    try {
      this._isOperating.set(true);
      const result = await window.electronAPI!.daemon.restart();
      if (!result.success) {
        throw new Error(result.error);
      }
      this.showMessage("Daemon restarted successfully", "success");
      await this.refreshStatus();
    } catch (error) {
      console.error("Failed to restart daemon:", error);
      this.showMessage("Failed to restart daemon", "error");
    } finally {
      this._isOperating.set(false);
    }
  }

  async refreshSystemServiceStatus(): Promise<void> {
    try {
      const result = await window.electronAPI!.daemon.getSystemServiceStatus();
      if (result.success) {
        this._systemServiceStatus.set(result.data);
      }
    } catch (error) {
      console.error("Failed to get system service status:", error);
      this._systemServiceStatus.set({
        installed: false,
        platform: "unknown",
      });
    }
  ...
  async setupSystemService(): Promise<void> {
    try {
      this._isOperating.set(true);
      const result = await window.electronAPI!.daemon.setupSystemService();
      if (!result.success) {
        throw new Error(result.error);
      }
      this.showMessage("System service setup successfully", "success");
      await this.refreshSystemServiceStatus();
    } catch (error) {
      console.error("Failed to setup system service:", error);
      this.showMessage(
        "Failed to setup system service: " + (error as Error).message,
        "error",
      );
    } finally {
      this._isOperating.set(false);
  ...
  async removeSystemService(): Promise<void> {
    try {
      this._isOperating.set(true);
      const result = await window.electronAPI!.daemon.removeSystemService();
      if (!result.success) {
        throw new Error(result.error);
      }
      this.showMessage("System service removed successfully", "success");
      await this.refreshSystemServiceStatus();
    } catch (error) {
      console.error("Failed to remove system service:", error);
      this.showMessage(
        "Failed to remove system service: " + (error as Error).message,
        "error",
      );
    } finally {
      this._isOperating.set(false);
  ...
  async loadLogs(): Promise<void> {
    try {
      this._isOperating.set(true);
      const result = await window.electronAPI!.daemon.getLogs(100);
      if (result.success) {
        this._daemonLogs.set(result.data || []);
        this.showMessage("Logs loaded successfully", "success");
      } else {
        throw new Error(result.error);
      }
    } catch (error) {
  ...
  async clearLogs(): Promise<void> {
    try {
      this._isOperating.set(true);
      const result = await window.electronAPI!.daemon.clearLogs();
      if (result.success) {
        this._daemonLogs.set([]);
  ...
  private buildDaemonConfig(): DaemonConfig {
    const config: DaemonConfig = {
      ...
      reportFormat: this.reportFormatInput as
        | "email"
        | "plain"
        | "markdown"
        | "json",
