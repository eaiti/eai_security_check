            ...
            <div class="verification-section">
              ...
              @if (singleVerificationResult() !== null) {
                <div
                  ...
                  [class]="getSingleVerificationClass()"
                >
...
export class ReportVerificationComponent implements OnInit {
  ...
  readonly singleVerificationResult =
    this._singleVerificationResult.asReadonly();
  ...
  selectDirectory(): void {
    ...
    input.addEventListener("change", (event) => {
      const files = (event.target as HTMLInputElement).files;
      if (files) {
        const reportFiles = Array.from(files).filter(
          (file) =>
  ...
  private addFilesToBulkList(files: File[]): void {
    ...
    this.showMessage(
      `Added ${uniqueFiles.length} files for bulk verification`,
      "success",
    );
  }
  ...
  async verifyBulkReports(): Promise<void> {
    ...
    this.showMessage(
      `Bulk verification completed: ${results.length} files processed`,
      "success",
    );
  }
