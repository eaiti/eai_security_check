      ...
      <div class="status-card">
        ...
        <div class="status-grid">
          ...
          <div class="status-item">
            ...
            <span class="status-value config-path">{{
              systemStatus().configDirectory
            }}</span>
          </div>
          <div class="status-item">
            <span class="status-label">Reports Directory:</span>
            <span class="status-value config-path">{{
              systemStatus().reportsDirectory
            }}</span>
          </div>
        </div>
...
export class DashboardComponent implements OnInit {
  ...
  private async loadRecentReports(): Promise<{
    recentReports: ReportHistory[];
    reportSource: "localStorage" | "fileSystem" | "mock";
  }> {
    try {
      if (!this.electronService.isElectron()) {
        return { recentReports: [], reportSource: "mock" };
      }

      // Load unified reports from ReportService
      const unifiedReports = this.reportService.getUnifiedReports();

      return {
        recentReports: unifiedReports.slice(0, 5), // Show last 5 reports
        reportSource:
          unifiedReports.length > 0
            ? unifiedReports.some((r) => r.source === "fileSystem")
              ? "fileSystem"
              : "localStorage"
            : "mock",
      };
  ...
  private async getConfigDirectory(): Promise<string> {
    try {
      if (this.electronService.isElectron()) {
        ...
        return (
          (await this.electronService.getConfigDirectory?.()) ||
  ...
  private async getReportsDirectory(): Promise<string> {
    try {
      if (this.electronService.isElectron()) {
        // In real implementation, this would come from electron service
        return (
          (await this.electronService.getReportsDirectory?.()) || "~/reports"
        );
      }
    } catch (error) {
  ...
  async viewReport(report: ReportHistory): Promise<void> {
    try {
      ...
      if (report.reportPath) {
        const reportData = await this.electronService.loadReportFromPath(
          report.reportPath,
        );
