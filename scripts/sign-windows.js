...
function checkRequirements() {
  ...
  if (os.platform() === "win32") {
    ...
    if (!signtoolPath) {
      try {
        ...
        // signtool.exe not found in common paths or PATH
      }
    }
  }
  ...
  if (!signtoolPath) {
    try {
      ...
      throw new Error(
        "Neither signtool.exe nor osslsigncode found. Please install Windows SDK or osslsigncode.",
      );
    }
  } else {
  ...
  if (!certFile && !certThumbprint) {
    throw new Error(
      "Either WINDOWS_CERT_FILE or WINDOWS_CERT_THUMBPRINT must be set",
    );
  }
...
function createChecksumFile() {
  ...
  try {
    ...
    if (os.platform() === "win32") {
      ...
      output = execSync(
        `powershell -Command "Get-FileHash '${EXECUTABLE_PATH}' -Algorithm SHA256 | Select-Object -ExpandProperty Hash"`,
        { encoding: "utf8" },
      );
      output = output.trim().toLowerCase();
    } else {
      // Use sha256sum on Unix-like systems
      output = execSync(`sha256sum "${EXECUTABLE_PATH}"`, {
        encoding: "utf8",
        cwd: BIN_DIR,
      });
...
function main() {
  try {
    ...
    console.warn(
      "[Windows Signing] WARNING: Users may see security warnings when running the executable",
    );
    console.warn(
      "[Windows Signing] INFO: To enable code signing, configure Windows certificates and signing tools",
    );
    ...
    try {
      ...
      console.warn(
        `[Windows Signing] WARNING: Could not create checksum file: ${checksumError.message}`,
      );
    }
