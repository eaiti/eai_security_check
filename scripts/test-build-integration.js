...
function testWindowsBuild() {
  ...
  try {
    ...
    if (!fs.existsSync(expectedPath)) {
      throw new Error(
        `Windows executable not found at expected path: ${expectedPath}`,
      );
    }
    ...
    log(
      `✅ Windows executable created successfully: ${expectedPath} (${stats.size} bytes)`,
    );
    return true;
  } catch (e) {
...
function testWindowsSigning() {
  ...
  try {
    ...
    log(
      `✅ Windows signing failsafe worked correctly: created ${checksumPath}`,
    );
